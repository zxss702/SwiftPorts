...
extension Repository {
    ...
    public func configGet(_ name: String, scope: ConfigScope? = nil) throws -> String? {
    ...
    public func configSet(_ name: String, _ value: String, scope: ConfigScope = .local) throws {
        var cfg: OpaquePointer?
        try check(git_repository_config(&cfg, repo))
        defer { git_config_free(cfg) }
    ...
    @discardableResult
    public func configUnset(_ name: String, scope: ConfigScope = .local) throws -> Bool {
        var cfg: OpaquePointer?
        try check(git_repository_config(&cfg, repo))
        defer { git_config_free(cfg) }
    ...
    public func configList() throws -> [(name: String, value: String)] {
        var cfg: OpaquePointer?
        try check(git_repository_config(&cfg, repo))
        defer { git_config_free(cfg) }
