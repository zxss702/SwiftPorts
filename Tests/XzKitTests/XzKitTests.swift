...
@Suite struct XzKitTests {
    ...
    @Test func roundTripData() async throws {
        ...
        let compressed = try await Xz.compress(original)
        // xz magic: FD 37 7A 58 5A 00 ('\xfd7zXZ\0')
        #expect(compressed.starts(with: [0xFD, 0x37, 0x7A, 0x58, 0x5A, 0x00]))
        let decompressed = try await Xz.decompress(compressed)
    ...
    @Test func compressFileProducesXzSuffix() async throws {
        let work = tempDir()
        defer { try? FileManager.default.removeItem(at: work) }
        let source = work.appendingPathComponent("payload.txt")
        try Data("payload\n".utf8).write(to: source)

        let dest = try await Xz.compressFile(at: source, keepInput: true)
    ...
    @Test func decompressRejectsTruncated() async throws {
        ...
        await #expect(throws: XzKitError.self) {
            _ = try await Xz.decompress(truncated)
        }
    }
}
