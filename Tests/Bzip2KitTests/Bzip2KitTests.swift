...
@Suite struct Bzip2KitTests {
    ...
    @Test func roundTripData() async throws {
        ...
        let compressed = try await Bzip2.compress(original)
        // bzip2 magic: 0x42 0x5A 0x68 ('BZh')
        #expect(compressed.starts(with: [0x42, 0x5A, 0x68]))
        let decompressed = try await Bzip2.decompress(compressed)
    ...
    @Test func compressFileProducesBz2Suffix() async throws {
        let work = tempDir()
        defer { try? FileManager.default.removeItem(at: work) }
        let source = work.appendingPathComponent("payload.txt")
        try Data("payload\n".utf8).write(to: source)

        let dest = try await Bzip2.compressFile(at: source, keepInput: true)
    ...
    @Test func decompressRejectsTruncated() async throws {
        ...
        await #expect(throws: Bzip2KitError.self) {
            _ = try await Bzip2.decompress(truncated)
        }
    }
}
